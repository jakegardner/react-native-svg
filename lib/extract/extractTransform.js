...
export default function (props) {
    const matrix = props.matrix || props.transform && props.transform.matrix;
    if (matrix) {
      if (!_.isArray(matrix) || matrix.length !== 6) {
        throw new Error('2d matrix transform must be an array of length six');
      }
      return matrix;
    }
